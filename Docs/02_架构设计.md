...
版本：v0.2；日期：2026-09-15。本文描述目标架构，不是已实现状态。
...
| UFighterAttributeSet | UAttributeSet | 生命、行动资源、能量、约束及必要的变化处理 |
...
| WBP_TrainingHUD | UMG | 双方属性、技能冷却、设置与统计 |
...
| AttributeSet | Health、MaxHealth、ActionResource、MaxActionResource、Energy、MaxEnergy |
...
当前建议 GA：MeleeCombo、Guard、Dodge、Throw、ActionCancel、Skill01、Skill02、Ultimate、HitReact、Knockdown、Death。

首个实机角色已确认为石流龙。建议配置 `DA_Fighter_Ishigori`，将 Skill01 映射为短突进重拳、Skill02 映射为基础咒力炮击、Ultimate 映射为强化蓄力炮击。以上为拟用资产名与默认映射，尚未创建；玩家和对手共用定义，各自拥有独立 ASC 和能力实例。公共 Character 不承担角色专属技能编排，详见 [角色方案](06_石流龙角色方案.md)。
...
- 石流龙首版炮击：默认采用发射时的一次即时射线结算，镜头射线确定目标点，再从头部炮口 Socket 向目标点检测最近阻挡；只有实际命中合法目标才进入统一结算。射线不穿过墙体，光束特效不重复造成伤害。
- 范围技能：球形或盒形查询。
- 命中去重键：攻击实例 + 命中段 + 目标。
...
强化炮击采用有限时长蓄力、到时发射的默认流程：激活成功提交能量消耗，发射前中断不产生伤害；正常战斗默认不退还已提交消耗，训练重置恢复初始属性。蓄力提示、射线和特效由同一能力实例管理；中断、死亡与重置必须清理延迟发射。它不是领域展开，也不引入必中或领域结界系统。
