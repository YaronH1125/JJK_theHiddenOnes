...
try:
 ...
 result=run(root/'Scripts/M6_acceptance.py');assert result['success'],result
 deadline=time.monotonic()+300
 while time.monotonic()<deadline:
  state=json.loads((root/'Saved/M6_acceptance.json').read_text(encoding='utf-8'))
  if state['status']!='running':break
  time.sleep(1)
 else:raise TimeoutError('M6 runner timeout')
 state['snapshot']={str(p.relative_to(root)).replace('\\','/'):hashlib.sha256(p.read_bytes()).hexdigest() for folder,pattern in [('Source','*.h'),('Source','*.cpp'),('Content/Training','*.uasset'),('Content/Maps','L_TrainingArena.umap'),('Scripts','M6*.py')] for p in (root/folder).rglob(pattern)}
 name='M6_Acceptance.json' if state['status']=='passed' else 'M6_Acceptance_failed_'+time.strftime('%H%M%S')+'.json'
 ...
 assert state['status']=='passed' and all(c['passed'] for c in state['checks'])
finally:
 if running():mcp.tool(editor,'StopPIE')
