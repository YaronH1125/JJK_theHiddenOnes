...
try:
 ...
 result=run(root/'Scripts/M5_demo.py');assert result['success'],result
 deadline=time.monotonic()+300
 while time.monotonic()<deadline:
  state=json.loads((root/'Saved/M5_demo.json').read_text(encoding='utf-8'))
 ...
 name='M5_Demo.json' if state['status']=='passed' else 'M5_Demo_failed_'+time.strftime('%H%M%S')+'.json'
 ...
 assert state['status']=='passed' and len(state['checks'])==12 and all(c['passed'] for c in state['checks'])
finally:
 if running():mcp.tool(editor,'StopPIE')
