"""Run M3 rules and M2 lifecycle regression in a fresh PIE; preserve every failed report."""
...
try:
    ...
    result=run(root/'Scripts/M3_acceptance.py');assert result['success'],result
    deadline=time.monotonic()+520
    while time.monotonic()<deadline:
        state=json.loads((root/'Saved/M3_acceptance.json').read_text(encoding='utf-8'))
    ...
    state['snapshot']={str(p.relative_to(root)).replace('\\','/'):hashlib.sha256(p.read_bytes()).hexdigest()
        for folder,pattern in [('Source','*.h'),('Source','*.cpp'),('Content/Training','*.uasset'),('Scripts','M3*.py')]
        for p in (root/folder).rglob(pattern)}
    path=root/'Docs/开发过程/验收记录'/('M3_Acceptance.json' if state['status']=='passed' else 'M3_Acceptance_failed_'+time.strftime('%H%M%S')+'.json')
    ...
    assert state['status']=='passed' and len(state['checks'])>=100 and all(c['passed'] for c in state['checks'])
finally:
    if running():mcp.tool(editor,'StopPIE')
