...
try:
    ...
    while time.monotonic()<deadline:
        try: state=json.loads((root/'Saved/M3_acceptance.json').read_text(encoding='utf-8'))
    ...
    path=root/'Docs/开发过程/验收记录'/('M4_M3_Regression.json' if state['status']=='passed' else 'M4_M3_Regression_failed_'+time.strftime('%H%M%S')+'.json')
