...
parser=argparse.ArgumentParser();parser.add_argument('--nullrhi',action='store_true');parser.add_argument('--report-prefix',default='M5');args=parser.parse_args()
assert args.report_prefix.replace('_','').isalnum()
...
log=runtime/(args.report_prefix+('_Package_nullrhi.log' if args.nullrhi else '_Package_graphics.log'))
...
if not args.nullrhi:
 ...
 shutil.copy2(shots[0],evidence/(args.report_prefix+'_独立包胜负.png'))
...
(evidence/(args.report_prefix+('_Package_nullrhi.json' if args.nullrhi else '_Package_graphics.json'))).write_text(json.dumps(state,ensure_ascii=False,indent=2),encoding='utf-8')
